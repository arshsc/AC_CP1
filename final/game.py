...
def servant_combat(player_health, player_stamina, player_weapon_chosen):
    ...
    while True:
        fight_confirm = input("\n\nAre you sure you want to fight the ghost? (yes/no): ").strip().lower()
    ...
    while ghost_health > 0 and player_health > 0:
        ...
        if turn == 1:
            ...
            if ghost_health <= 0:
                ...
                if "Ghost Axe" in inventory["Weapons"]:
                    inventory["Weapons"]["Ghost Axe"] += 10  # increase damage
                    print("\nYour Ghost Axe glows! Its power has increased by +10 DMG!")
...
# miner
def miner_combat(player_health, player_stamina, player_weapon_chosen):
    global miner_defeated

    if miner_defeated:
        print("\nThe miner has already been defeated. There’s nothing left here.")
        return player_health, player_stamina, False
    ...
    print("\n" * 100)
    print("\nA massive ghost miner steps out from the shadows.")
    print("\nHe grunts at you.")
    print(f"\nWeapon Equipped: {player_weapon_chosen} (+{base_player_damage} DMG)")

    while True:
        fight_confirm = input("\n\nAre you sure you want to fight the miner? (yes/no): ").strip().lower()
        if fight_confirm in ["yes", "y"]:
            print("\n" * 100)
            ...
            print("\n" * 100)
            print("\nYou back away from the miner and retreat.")
            return player_health, player_stamina, False
        else:
            print("Please type Yes or No.")
    ...
    while miner_health > 0 and player_health > 0:
        if turn == 1:
            ...
            if choice == "quick attack":
                damage = base_player_damage + random.randint(5, 20)
                miner_health -= damage
                print("\n" * 100)
                print(f"\nYou strike the miner for {damage} damage!")
                player_stamina -= 1
                player_acted = True
                ...
                damage = base_player_damage + random.randint(15, 40)
                miner_health -= damage
                print("\n" * 100)
                print(f"\nYou unleash a heavy blow for {damage} damage!")
                player_stamina -= 3
                player_acted = True
                ...
                available_consumables = []
                if len(inventory["Consumables"]["Bandage"]) > 0:
                    available_consumables.append(
                        f"Bandage (+{inventory['Consumables']['Bandage'][0]} HP) x{len(inventory['Consumables']['Bandage'])}")
                    
                if len(inventory["Consumables"]["Energy Drink"]) > 0:
                    available_consumables.append(
                        f"Energy Drink (+{inventory['Consumables']['Energy Drink'][0]} Stamina) x{len(inventory['Consumables']['Energy Drink'])}")
            ...
            if miner_health <= 0:
                print("\nThe miner collapses to the ground. The gems scatter across the floor.")
            ...
            if attack_type <= 3:
                ...
                print("The miner swings his massive pickaxe!")
            else:
                damage = random.randint(10, 30)
                print("The miner slams the ground, shaking the tunnel!")

            if random.randint(1, 100) <= 30:
                print("You dodge the miner's attack!")
                damage = 0
...
def mansion(master_bedroom_searched, vault_searched, garden_searched, player_weapon_chosen, player_health, player_stamina, servant_defeated):
    ...
    while location == "Mansion":
        ...
        if search == "master bedroom":
            if not master_bedroom_searched:
            ...
            if not vault_searched:
                ...
                while True:
                    ...
                    if weapon_choice in ["Dagger", "Ghost Axe"]:
            ...
            leave = input("\n\nDo you want to leave the garden and return to the mansion? (yes/no):  ").strip().lower()
            if leave in ["yes", "y"]:
                print("\n" * 100)
                continue
            elif leave in ["no", "n"]:
        ...
        while True:
            if search == "kitchen":
                ...
                if cell_searched == False:
                    cell_searched = True
                    print("\n" * 100)
                    print("\nYou walk into one of the only cells not blocked. You search the small table and under the bed. You find a single wire and a bandage under the bed. \n\n(+) Wire\n\n(+) Bandage")
        ...
        while True:
            ...
            if search == "classroom":
                if classroom_searched == False:
                    ...
                    print("\n" * 100)
                    print("\nYou enter the classroom. The roof seems that it is about to cave in. On one of the desks, there is 2 energy drinks that seems new. You pick it up and put it in your backpack.\n\n(+) Energy Drink\n\n(+) Energy Drink")
                    inventory["Consumables"]["Energy Drink"].append(4)
                    inventory["Consumables"]["Energy Drink"].append(4)
                    break
                elif classroom_searched == True:
                    print("\n" * 100)
                    print("\nYou already searched the classroom, you picked up the 2 energy drinks and nothing else is to be found.")
                    break
                ...
                if school_library_searched == False:
                    ...
                    print("\n" * 100)
                    print("\nYou walk into the tiny library. The bookshelves are on the floor, and many books scattered through the floor. You see a bandage tucked under the bookshelf.\n\n(+) Bandage")
                    inventory["Consumables"]["Bandage"].append(10)
                    break
    ...
    while location == "Entrance of the Mines":
        ...
        if search == "cart station":
            if not cart_station_searched:
                ...
                print("\n" * 100)
                print("You turn the corner to the cart station. A bunch of old carts are left here. You need one to go down into the mines. You put a cart onto the rails. You also grab a 2 left over bandages in the small cubby.\n\n(+) Bandage\n\n(+) Bandage")
                inventory["Consumables"]["Bandage"].append(10)
                inventory["Consumables"]["Bandage"].append(10)
                cart_on_rails = True
            else:
                print("\n" * 100)
                print("You already put a cart on the rails and grabbed 2 bandages.")
            ...
            if not cart_on_rails:
                ...
                if go_into_mines in ["yes", "y"]:
                    ...
                    if wire_count >= 3 and battery_found and rail_key:
                        ...
                        for item in missing:
                            print(f"     - {item}")
                        print("\nYou cannot enter the mines yet.")
...
def underground_railroad(wire_count, battery_found, miner_health, player_health, player_stamina, player_weapon_chosen):
    ...
    print("\nYou connect the 3 wires and the battery to the power system. The gates open.")
    ...
    while location == "Underground Railroad":
        ...
        if choice == "exit":
            ...
            if not miner_defeated:
                player_health, player_stamina, fought = miner_combat(player_health, player_stamina, player_weapon_chosen)
                if player_health <= 0:
                    return wire_count, battery_found, miner_health, player_health, player_stamina, miner_defeated
                ...
                print("\nThe tunnels are quiet now. The miner has already been defeated.")
...
# inventory
def show_inventory(player_weapon_chosen, player_health, player_stamina):
    max_health = 50
    max_stamina = 10

    while True:
        print("\n" * 100)
        print(f"Health: {player_health}/{max_health}")
        print(f"Stamina: {player_stamina}/{max_stamina}")
        ...
        if not inventory["Consumables"]:
            ...
            for item, effects in inventory["Consumables"].items():
                if len(effects) == 0:
                    ...
                    unit = "HP" if item.lower() == "bandage" else "Stamina"
        ...
        use_choice = input("\nWould you like to use a consumable? (yes/no): ").lower().strip()
        if use_choice in ["no", "n"]:
            break

        elif use_choice in ["yes", "y"]:
            if not inventory["Consumables"]:
                print("You have no consumables to use!")
                break

            use_input = input("Which consumable would you like to use?: ").strip()

            matched_item = None
            for item in inventory["Consumables"]:
                if item.lower() == use_input.lower():
                    matched_item = item
                    break

            if matched_item and len(inventory["Consumables"][matched_item]) > 0:
                effect_value = inventory["Consumables"][matched_item][0]
                if matched_item.lower() == "bandage":
                    if player_health >= max_health:
                        print("\n" * 100)
                        print("You're already at full health!")
                    else:
                        player_health += effect_value
                        if player_health > max_health:
                            player_health = max_health
                        inventory["Consumables"][matched_item].pop(0)
                        print(f"\nYou healed {effect_value} HP. (Now {player_health}/{max_health})")
                    break
                else:
                    if player_stamina >= max_stamina:
                        ...
                        player_stamina += effect_value
                        if player_stamina > max_stamina:
                            player_stamina = max_stamina
                        inventory["Consumables"][matched_item].pop(0)
                        print(f"\nYou restored {effect_value} Stamina. (Now {player_stamina}/{max_stamina})")
                    break
            else:
                print("\n" * 100)
                print("Invalid consumable or none left.")
                continue
        else:
            print("\n" * 100)
    ...
    return player_health, player_stamina


# room input and inventory function
def room_input(locations, player_weapon_chosen, player_health, player_stamina):
    ...
    while True:
        ...
        if choice.lower() in ["inventory", "inv", "i"]:
            print("\n" * 100)
            player_health, player_stamina = show_inventory(player_weapon_chosen,player_health,player_stamina)
            continue
        ...
        return choice, player_health, player_stamina
...
while True:
    ...
    while True:
        ...
        location, player_health, player_stamina = room_input(locations,player_weapon_chosen,player_health,player_stamina)
        ...
        if location == "Trailer Home":
            ...
            if lever not in inventory["Items"]:
                ...
                if enter_mines and not miner_defeated:
                    wire_count = inventory["Items"].count("Wire")
                    has_battery = "Battery" in inventory["Items"]
                    ...
                    if miner_defeated:
                        while True:
                            replay = input("\nCongratulations! You've defeated the miner. Would you like to replay the game? (yes/no): ").strip().lower()
                            if replay in ["yes", "y"]:
                                print("\nRestarting the game...\n")
                                break
                            elif replay in ["no", "n"]:
